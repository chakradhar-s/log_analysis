...
def question1():
    ...
    substring(path from '([a-zA-z\-]+)$') article from log where status similar 
    to '(2|3)%' group by path having path like '%/article%' order by totalviews 
    desc limit 3''')
    posts = c.fetchall()
    db.close()
    return posts
...
def question2():
    ...
    having path like '%/article%' order by total desc) dt  where au.id=ar.author
    and dt.sub=ar.slug''')
    posts = c.fetchall()
    db.close()
    return posts
...
def question3():
    ...
    (select lg.time::date,count(status) as error_requests,status as errorcodes 
    from log lg group by lg.time::date,status having status not 
    similar to '(2|3)%') as er,(select lg.time::date, count(status) 
    as totalrequests from log lg group by lg.time::date) as pas where 
    er.time=pas.time and 
